...
type InvestTab = 'portfolio' | 'goals';
...
function FxSettingsCard({
  displayCurrency, fxRates, onCurrencyChange, onRatesChange, onUpdate, fetching, lastUpdated, fetchError,
}: {
  ...
  const commitRates = () => {
    ...
    if (!isNaN(USD) && USD > 0 && !isNaN(HKD) && HKD > 0) onRatesChange({ USD, HKD });
  };
  ...
  return (
    <Card gap={MS.sm}>
      ...
      <View style={styles.ccyRow}>
        {CURRENCIES.map((ccy) => (
          <Pressable
            ...
            <Text style={[styles.ccyBtnText, displayCurrency === ccy && styles.ccyBtnTextActive]}>{ccy}</Text>
          </Pressable>
        ))}
      </View>
      ...
      {fetchError
        ...
        : lastUpdated != null
          ? <Text style={styles.lastUpdatedText}>{t('portfolio.lastUpdated', { time: fmtTime(lastUpdated) })}</Text>
...
function makeFxStyles(C: AppTheme) {
  return StyleSheet.create({
    ...
    updateBtn: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: MS.md, paddingVertical: 5, borderRadius: MR.md, borderWidth: 1, borderColor: C.emerald + '60', backgroundColor: C.emerald + '12', minWidth: 36, justifyContent: 'center' },
    ...
    ccyBtn: { paddingHorizontal: MS.lg, paddingVertical: MS.sm, borderRadius: MR.lg, borderWidth: 1.5, borderColor: C.line, backgroundColor: C.card },
    ...
    rateInput: { flex: 1, backgroundColor: C.bg, borderWidth: 1, borderColor: C.line, borderRadius: MR.md, paddingHorizontal: MS.md, paddingVertical: MS.sm, fontSize: 14, fontFamily: MF.medium, color: C.ink, textAlign: 'right' },
...
// ─── Holding card (full + compact) ───────────────────────────────────────────

function HoldingCard({
  holding, displayCurrency, fxRates, onPress, compact = false,
}: {
  holding: Holding;
  displayCurrency: HoldingCurrency;
  fxRates: FxRates;
  ...
  if (compact) {
    return (
      <Pressable
        style={({ pressed }) => [styles.holdingCompact, pressed && styles.holdingCardPressed]}
        onPress={onPress}>
        <View style={[styles.typeChip, { backgroundColor: typeColor + '1A', alignSelf: 'flex-start' }]}>
          <Text style={[styles.typeChipText, { color: typeColor }]}>
            {t(`portfolio.type${holding.assetType}`)}
          </Text>
        </View>
        <Text style={styles.holdingNameSmall} numberOfLines={2}>{holding.name}</Text>
        <Text style={styles.holdingValueSmall}>{fmtCcy(holding.currentValue, holdingCcy)}</Text>
        {gainLoss !== null && gainPct !== null && (
          <Text style={[styles.gainLineSmall, { color: isGain ? C.emerald : C.clay }]}>
            {isGain ? '▲' : '▼'} {Math.abs(gainPct).toFixed(1)}%
          </Text>
        )}
        {convertedValue !== null && (
          <Text style={styles.convertedSmall}>≈ {fmtCcy(convertedValue, displayCurrency)}</Text>
        )}
      </Pressable>
    );
  }
      ...
      <View style={styles.holdingTop}>
        <View style={{ flex: 1 }}>
          <View style={styles.holdingMeta}>
            ...
            {holding.units != null && (
              <Text style={styles.holdingUnits}>{holding.units} {t('portfolio.units')}</Text>
            )}
          </View>
    ...
    <Pressable
      ...
      <View style={styles.goalHeader}>
        ...
        <View style={styles.goalInfo}>
          ...
          <Text style={styles.goalSub}>{fmt(goal.saved)} / {fmt(goal.target)}</Text>
        </View>
...
export default function InvestScreen() {
  ...
  const triggerFetch = async () => {
    ...
    try {
      ...
      } catch { /* crypto fetch is best-effort */ }
  ...
  const allocationSegments = (Object.entries(allocationMap) as [AssetType, number][])
    .map(([type, val]) => ({ label: t(`portfolio.type${type}`), val, color: ASSET_COLORS[type] }))
    .sort((a, b) => b.val - a.val);

  const portfolioValueDisplay = data.portfolioValueDisplay ?? 0;
  const topAsset = allocationSegments[0];
  const topAssetPct = topAsset && portfolioValueDisplay > 0
    ? Math.round((topAsset.val / portfolioValueDisplay) * 100)
    : 0;

  const totalGainLoss = holdings.reduce((sum, h) => {
    if (h.units != null && h.buyPrice != null) return sum + (h.currentValue - h.units * h.buyPrice);
    return sum;
  }, 0);
  const hasGainData = holdings.some((h) => h.units != null && h.buyPrice != null);
  ...
  const TAB_OPTIONS: { key: InvestTab; label: string }[] = [
  ...
  return (
    <View style={styles.root}>
      <ScrollView
        ...
        {/* Tab switcher */}
        <TabSelector tabs={TAB_OPTIONS} active={tab} onSelect={setTab} activeColor={C.emerald} />
        ...
        {tab === 'portfolio' && (
          <>
            ...
            <HeroCard
              ...
              label={t('portfolio.heroLabel')}
              amount={fmtCcy(portfolioValueDisplay, displayCcy)}
              amountFontSize={38}>
              <View style={styles.heroLine} />
              <Text style={styles.heroSub}>{t('portfolio.holdingsCount', { n: holdings.length })}</Text>
            </HeroCard>

            {/* Stats row */}
            <View style={styles.statsRow}>
              <StatPill value={holdings.length} label="Holdings" />
              <StatPill value={topAsset?.label ?? '—'} label={topAssetPct > 0 ? `${topAssetPct}% largest` : 'No data'} numberOfLines={1} />
              <StatPill
                value={hasGainData ? `${totalGainLoss >= 0 ? '+' : ''}${fmt(totalGainLoss)}` : '—'}
                label="Gain / Loss"
                valueColor={hasGainData ? (totalGainLoss >= 0 ? C.emerald : C.clay) : undefined}
              />
            </View>
            ...
            {allocationSegments.length > 0 && (
              <Card gap={MS.md}>
                ...
                {allocationSegments.map((s) => {
                  const pct = portfolioValueDisplay > 0
                    ? Math.round((s.val / portfolioValueDisplay) * 100)
                    : 0;
                  const maxVal = allocationSegments[0]?.val ?? 1;
                  return (
                    <View key={s.label} style={styles.allocRow}>
                      ...
                      <View style={styles.allocBody}>
                        <View style={styles.allocTop}>
                          ...
                          <Text style={styles.allocAmt}>{fmtCcy(s.val, displayCcy)}</Text>
                        </View>
                        <ProgressBar value={Math.max(4, (s.val / maxVal) * 100)} color={s.color} />
            ...
            {/* Collapsible FX settings */}
            <Pressable style={styles.fxToggleRow} onPress={() => setFxOpen((v) => !v)}>
              <Text style={styles.fxToggleIcon}>⚙</Text>
              <View style={{ flex: 1 }}>
                <Text style={styles.fxToggleTitle}>{t('portfolio.fxTitle')}</Text>
                <Text style={styles.fxToggleRates}>
                  USD {fxRates.USD} · HKD {fxRates.HKD} RM · {displayCcy}
                </Text>
              </View>
              <Text style={styles.fxToggleArrow}>{fxOpen ? '▲' : '▼'}</Text>
            </Pressable>
        ...
        {tab === 'goals' && (
          <>
            {/* Goals hero */}
            <HeroCard
              gradient={[C.indigo, C.indigo + 'CC']}
              label={t('invest.heroLabel')}
              ...
              <View style={styles.heroRow}>
                ...
                <View>
                  ...
                  <Text style={styles.heroVal}>{fmt(Math.max(0, totalTarget - totalSaved))}</Text>
                </View>
                <View>
              ...
              <View style={styles.heroMeterBg}>
                <View style={[styles.heroMeterFill, { width: `${overallPct}%` }]} />
              </View>
            </HeroCard>
            ...
            <View style={styles.surplusCard}>
              ...
              <View style={styles.surplusTip}>
                <Text style={styles.surplusTipText}>
                  {t('invest.surplusTip', { amt: fmt(MONTHLY_SURPLUS * 0.25) })}
                </Text>
              </View>
            </View>
...
function makeStyles(C: AppTheme) {
  return StyleSheet.create({
    ...
    heroRow: { flexDirection: 'row', gap: MS.xl, marginTop: 2 },
    heroKey: { fontSize: 10, fontFamily: MF.medium, color: 'rgba(255,255,255,0.75)', textTransform: 'uppercase', letterSpacing: 0.5 },
    heroVal: { fontSize: 14, fontFamily: MF.bold, color: '#fff', marginTop: 3 },
    heroMeterBg: { height: 4, backgroundColor: 'rgba(255,255,255,0.25)', borderRadius: 2, overflow: 'hidden', marginTop: MS.md },
    heroMeterFill: { height: '100%', borderRadius: 2, backgroundColor: C.gold },
    ...
    sectionTitle: { fontSize: 12, fontFamily: MF.bold, color: C.muted, textTransform: 'uppercase', letterSpacing: 0.7 },
    addBtn: {
    ...
    holdingsGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: MS.sm },
    holdingCompact: {
      width: '48.5%',
    ...
    holdingCard: { backgroundColor: C.card, borderWidth: 1, borderColor: C.line, borderRadius: MR.xl, padding: MS.lg, gap: MS.sm },
    ...
    typeChipText: { fontSize: 10, fontFamily: MF.semiBold, letterSpacing: 0.3 },
    holdingUnits: { fontSize: 11, fontFamily: MF.regular, color: C.muted },
    holdingName: { fontSize: 16, fontFamily: MF.bold, color: C.ink },
    holdingNameSmall: { fontSize: 13, fontFamily: MF.bold, color: C.ink },
    holdingRight: { alignItems: 'flex-end' },
    holdingValue: { fontSize: 17, fontFamily: MF.bold, color: C.ink },
    holdingValueSmall: { fontSize: 14, fontFamily: MF.bold, color: C.ink },
    gainLine: { fontSize: 12, fontFamily: MF.semiBold, marginTop: 3 },
    gainLineSmall: { fontSize: 11, fontFamily: MF.semiBold },
    convertedSmall: { fontSize: 10, fontFamily: MF.regular, color: C.muted },
    costRow: { flexDirection: 'row', justifyContent: 'space-between', paddingTop: MS.sm, borderTopWidth: 1, borderTopColor: C.line },
    ...
    emptyCard: { backgroundColor: C.card, borderWidth: 1, borderColor: C.line, borderRadius: MR.xl, padding: MS.xl, alignItems: 'center' },
    ...
    // ── Allocation bars ──
    allocRow: { flexDirection: 'row', alignItems: 'flex-start', gap: MS.sm },
    allocDot: { width: 10, height: 10, borderRadius: 3, flexShrink: 0, marginTop: 4 },
    allocBody: { flex: 1 },
    allocTop: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 },
    allocLabel: { fontSize: 13, fontFamily: MF.medium, color: C.ink },
    allocAmt: { fontSize: 13, fontFamily: MF.bold, color: C.ink },
    allocPct: { fontSize: 11, fontFamily: MF.regular, color: C.muted, marginTop: 3 },
    ...
    fxToggleIcon: { fontSize: 16, color: C.muted },
    fxToggleTitle: { fontSize: 13, fontFamily: MF.semiBold, color: C.ink },
    fxToggleRates: { fontSize: 11, fontFamily: MF.regular, color: C.muted, marginTop: 2 },
    fxToggleArrow: { fontSize: 11, color: C.muted, fontFamily: MF.medium },
    ...
    // ── Goals tab ──
    surplusCard: { backgroundColor: C.card, borderWidth: 1, borderColor: C.line, borderRadius: MR.xl, padding: MS.lg, gap: MS.sm },
    surplusLeft: { gap: 2 },
    ...
    surplusTip: { backgroundColor: C.goldLight, borderWidth: 1, borderColor: C.goldBorder, borderRadius: MR.sm, paddingHorizontal: MS.md, paddingVertical: MS.xs },
    surplusTipText: { fontSize: 12, fontFamily: MF.medium, color: C.goldText },

    goalCard: { backgroundColor: C.card, borderWidth: 1, borderColor: C.line, borderRadius: MR.xl, padding: MS.lg, gap: MS.md },
    ...
    calcRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: MS.sm, borderBottomWidth: 1, borderBottomColor: C.line },
